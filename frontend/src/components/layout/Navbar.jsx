          ...
          <div className="hidden md:flex items-center gap-3">
            {isAuthenticated ? (
              <div className="flex items-center gap-3">
                ...
                <button onClick={logout} className="flex items-center gap-1 text-sm text-red-500 hover:text-red-700 font-medium">
        ...
        <div className="md:hidden bg-white border-t border-gray-100 px-4 py-4 flex flex-col gap-4 animate-fade-in">
          {navLinks.map(({ to, label }) => (
            <NavLink key={to} to={to} onClick={() => setOpen(false)}
              className={({ isActive }) => `font-semibold ${isActive ? 'text-primary-600' : 'text-gray-700'}`}>
              {label}
            </NavLink>
          ))}
          <div className="flex gap-3 pt-2 border-t border-gray-100">
            {isAuthenticated ? (
              <button onClick={() => { logout(); setOpen(false); }} className="text-red-500 font-medium text-sm">
                Logout
              </button>
            ) : (
              <>
                <Link to="/login" onClick={() => setOpen(false)} className="btn-secondary py-2 text-sm flex-1 text-center">Sign In</Link>
                <Link to="/register" onClick={() => setOpen(false)} className="btn-primary py-2 text-sm flex-1 text-center">Register</Link>
              </>
            )}
          </div>
        </div>
